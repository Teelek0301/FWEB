...
router.post("/login", async (req, res) => {
    try {
        ...
        const member = await db.collection("members").findOne({ name });

        if (!member) {
            return res.status(400).json({ status: "error", error: "Invalid username", user: false });
        }

        const isValidPassword = await bcrypt.compare(password, member.hashedPassword);
        ...
        try {
            if (checkTwoFa(sixDigitCode, member.secret) === true) {
                const token = jwt.sign({ name: member.name, id: member._id }, 'secret123');
